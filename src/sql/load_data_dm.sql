...
b.weight,
b.height,
from_unixtime(unix_timestamp(),'yyyy-MM-dd HH:mm:ss') as dm_date
from
qfbap_dwd.dwd_user a
join
qfbap_dwd.dwd_user_extend b
on a.user_id=b.user_id;
--导入dm_user_visit
insert overwirte table qfbap_dm.dm_user_visit
select 
   us.user_id
...
left join (
    ...
    select 
      user_id
       ...
       ,sum(dt7) day7_pc_cnt --7天访问次数
       ,sum(dt15) day15_pc_cnt-- 15天访问次数
       ,sum(dt30) month1_pc_cnt -- 30天访问次数
       ,sum(dt60) month2_pc_cnt-- 60天访问的次数
       ,sum(dt90) month3_pc_cnt -- 90天访问的次数
    ...
    from (
       select 
          ...
          (case when in_time >= date_sub('2018-12-25',59) then 1 end) dt60,
           (case when in_time >= date_sub('2018-12-25',79) then 1 end) dt90,
